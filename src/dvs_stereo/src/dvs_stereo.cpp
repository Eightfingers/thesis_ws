...
void DVSStereo::readEventArray(dvs_msgs::EventArray &event_array, cv::Mat &event_image, cv::Mat &event_image_polarity, cv::Mat &map1_x, cv::Mat &map1_y)
{
    // std::cout << "event_array.events.size() " << event_array.events.size() <<  "\n"; 
    for (int i = 0; i < event_array.events.size(); i++)
    {
        ...
        event_image.at<uchar>(row, col) += 1;
                ...
                else
                {
                    ...
                    for (int wy = -NN_block_size_ / 2; wy <= NN_block_size_ / 2; wy++)
                    {
                        for (int wx = -NN_block_size_ / 2; wx <= NN_block_size_ / 2; wx++)
                        {
                            ...
                            if (event_image.at<uchar>(y + wy, x + wx) != 0)
                            {
                                num_of_events++;
                    ...
                    if (noise) // remove that event
                    {
                        ...
                        event_image_polarity.at<uchar>(y, x) = -1;
    ...
    cv::remap(event_image, event_image, map1_x, map1_y, cv::INTER_LINEAR);
    cv::remap(event_image_polarity, event_image_polarity, map1_x, map1_y, cv::INTER_LINEAR);
    ...
    // std::cout << "Nearest neighbor & event slicing & rectification took: " << duration_NN.count() / 1000.0 << " milliseconds" << std::endl;
}
    ...
    cv::Mat &event_image_polarity_right)
{
    ...
    for (int y = half_block; y < event_image_left.rows - half_block; y += window_step_size_)
    {
        for (int x = half_block; x < event_image_left.cols - half_block; x += window_step_size_)
        {
            if (event_image_left.at<uchar>(y, x) == 0)
            {
                continue; // skip processing
            }
...
void DVSStereo::syncCallback(const dvs_msgs::EventArray::ConstPtr &msg1, const dvs_msgs::EventArray::ConstPtr &msg2)
{
    ...
    double left_ts = left_events_2.header.stamp.toSec(); 
    double right_ts = right_events_2.header.stamp.toSec();
    ...
    double left_start_ts_ = left_start.ts.toSec();
    double left_end_ts_ = left_end.ts.toSec();

    dvs_msgs::Event right_start = right_events_2.events.front();
    dvs_msgs::Event right_end = right_events_2.events.back();
    double right_start_ts_ = right_start.ts.toSec();
    double right_end_ts_ = right_end.ts.toSec();
