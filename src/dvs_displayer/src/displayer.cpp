...
namespace dvs_displayer
{
  ...
  Displayer::Displayer(ros::NodeHandle &nh, ros::NodeHandle nh_private) : nh_(nh)
  {
    ...
    else if (display_method_str == std::string("blackwhite"))
    {
      display_method_ = BLACKWHITE;
    }
    else
    {
  ...
  void Displayer::eventsCallback(const dvs_msgs::EventArray::ConstPtr &msg)
  {
    ...
    cv::Mat accumulated_events = cv::Mat(msg->height, msg->width, CV_8U, cv::Scalar(127));
    ...
    for (const dvs_msgs::Event &ev : msg->events)
    {
      if (ev.polarity == true)
      {
        ...
        accumulated_events.at<uint8_t>(ev.y, ev.x) = 255;
      }
      else
      {
        off_events.at<uint8_t>(ev.y, ev.x)++;
        accumulated_events.at<uint8_t>(ev.y, ev.x) = 0;
      }
    }
    ...
    else if (display_method_ == BLACKWHITE)
    {
      cv_image.image = cv::Mat(msg->height, msg->width, CV_8U);
      cv_image.encoding = "mono8";
      accumulated_events.copyTo(cv_image.image);
    }
    else
    {
    ...
    if (msg->events.size() > 10)
    {
      // Stats
      ros::Time start_event_ts = msg->events.at(0).ts;
      ros::Time end_event_ts = msg->events.back().ts;
      ros::Duration time_duration = end_event_ts - start_event_ts;
      double duration_in_seconds = time_duration.toSec();
      double event_rate = msg->events.size() / duration_in_seconds;
      static uint64_t total_events = 0;
      static double highest_event_rate = 0;

      total_events += msg->events.size();
      if (highest_event_rate < event_rate)
      {
        highest_event_rate = event_rate;
      }

      std::cout << "**Event size: " << msg->events.size() << std::endl;
      std::cout << "**Event rate/s: " << event_rate << std::endl;
      std::cout << "**Highest event rate/s: " << highest_event_rate << std::endl;
